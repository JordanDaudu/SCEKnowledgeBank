...
- **Pinned transitive deps (`pnpm.overrides` in root `package.json`).** The root override block force-bumps known-vulnerable transitive packages flagged by `runDependencyAudit()` to their patched versions: `brace-expansion` (≥2.0.3), `fast-uri` (≥3.1.2 — path-normalization + host-confusion bypasses), `lodash` (≥4.18.0, via recharts dev tree), `path-to-regexp` (≥8.4.0 — the only one riding in the production Express 5 tree, via `router`), `picomatch` (2.x ≥2.3.2 and 4.x ≥4.0.4, both in the vite/vitest dev tree), `postcss` (≥8.5.10), `qs` (≥6.15.2), `uuid` (≥11.1.1 — bumps `@google-cloud/storage`'s pinned 8.x and gaxios's 9.x; the v4 random-UUID API surface is unchanged), `yaml` (≥2.8.3). When adding a new dependency, re-run `runDependencyAudit()`; if a transitive dep regresses below one of these floors, widen the override range here rather than removing it.
